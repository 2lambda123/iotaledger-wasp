...
func (f *Field) Compile(s *Schema, fldNameDef, fldTypeDef *DefElt) error {
	...
	if !fldNameRegexp.MatchString(f.Name) {
		return ierrors.Errorf("invalid field name: %s at line %d", f.Name, fldNameDef.Line)
	}
	if !fldAliasRegexp.MatchString(f.Alias) {
		return ierrors.Errorf("invalid field alias: %s at line %d", f.Alias, fldNameDef.Line)
	}
	...
	return ierrors.Errorf("invalid field type: %s at line %d", fldType, fldTypeDef.Line)
}
...
func (f *Field) compileFieldType(fldTypeDef *DefElt) (string, error) {
	...
	case strings.HasPrefix(fldType, "map["): // is it a map?
		...
		if len(parts) != 2 {
			return "", ierrors.Errorf("expected map field type: %s at line %d", fldType, fldTypeDef.Line)
		}
		f.MapKey = strings.TrimSpace(parts[0])
		if !fldTypeRegexp.MatchString(f.MapKey) || f.MapKey == "Bool" {
			return "", ierrors.Errorf("invalid map key field type: %s at line %d", f.MapKey, fldTypeDef.Line)
		}
