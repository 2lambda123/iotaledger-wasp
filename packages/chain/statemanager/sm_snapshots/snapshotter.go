...
func (sn *snapshotterImpl) storeSnapshot(snapshotInfo SnapshotInfo, w io.Writer) error {
	...
	if err != nil {
		return ierrors.Errorf("failed writing block index %v: %w", snapshotInfo.StateIndex(), err)
	}

	trieRootBytes := snapshotInfo.Commitment().Bytes()
	err = writeBytes(trieRootBytes, w)
	if err != nil {
		return ierrors.Errorf("failed writing L1 commitment %s: %w", snapshotInfo.Commitment(), err)
	}

	err = sn.store.TakeSnapshot(snapshotInfo.TrieRoot(), w)
	if err != nil {
		return ierrors.Errorf("failed to store snapshot: %w", err)
	}
	return nil
}
...
func (sn *snapshotterImpl) loadSnapshot(snapshotInfo SnapshotInfo, r io.Reader) error {
	...
	if err != nil {
		return ierrors.Errorf("failed reading snapshot info: %w", err)
	}
	if !readSnapshotInfo.Equals(snapshotInfo) {
		return ierrors.Errorf("snapshot read %s is different than expected %v", readSnapshotInfo, snapshotInfo)
	}
	err = sn.store.RestoreSnapshot(readSnapshotInfo.TrieRoot(), r)
	if err != nil {
		return ierrors.Errorf("failed restoring snapshot: %w", err)
	}
	return nil
}
...
func readSnapshotInfo(r io.Reader) (SnapshotInfo, error) {
	...
	if err != nil {
		return nil, ierrors.Errorf("failed to read block index: %w", err)
	}
	if len(indexArray) != 4 { // Size of block index, which is of type uint32: 4 bytes
		return nil, ierrors.Errorf("block index is %v instead of 4 bytes", len(indexArray))
	}
	index := binary.LittleEndian.Uint32(indexArray)

	trieRootArray, err := readBytes(r)
	if err != nil {
		return nil, ierrors.Errorf("failed to read trie root: %w", err)
	}
	commitment, err := state.L1CommitmentFromBytes(trieRootArray)
	if err != nil {
		return nil, ierrors.Errorf("failed to parse L1 commitment: %w", err)
	}
...
func writeBytes(bytes []byte, w io.Writer) error {
	...
	if n != constLengthArrayLength {
		return ierrors.Errorf("only %v of total %v bytes of length written", n, constLengthArrayLength)
	}
	if err != nil {
		return ierrors.Errorf("failed writing length: %w", err)
	}

	n, err = w.Write(bytes)
	if n != len(bytes) {
		return ierrors.Errorf("only %v of total %v bytes of array written", n, len(bytes))
	}
	if err != nil {
		return ierrors.Errorf("failed writing array: %w", err)
	}
...
func readBytes(r io.Reader) ([]byte, error) {
	...
	lengthArray := w.Bytes()
	if err != nil {
		return nil, ierrors.Errorf("read only %v bytes out of %v of length, error: %w", read, constLengthArrayLength, err)
	}
	...
	array := w.Bytes()
	if err != nil {
		return nil, ierrors.Errorf("only %v of %v bytes of array read, error: %w", read, len(array), err)
	}
