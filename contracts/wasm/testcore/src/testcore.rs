...
pub fn func_check_context_from_full_ep(ctx: &ScFuncContext, f: &CheckContextFromFullEPContext) {
    ...
    ctx.require(
        f.params.contract_creator().value() == ctx.contract_creator(),
        "fail: contractCreator",
    );
}
...
pub fn func_pass_types_full(ctx: &ScFuncContext, f: &PassTypesFullContext) {
    ...
    ctx.require(
        f.params.hname().value() == ctx.utility().hash_name(PARAM_HNAME),
        "Hname wrong",
    );
...
pub fn func_test_event_log_deploy(ctx: &ScFuncContext, _f: &TestEventLogDeployContext) {
    ...
    ctx.deploy_contract(
        &program_hash,
        CONTRACT_NAME_DEPLOYED,
        "test contract deploy log",
        None,
    );
}
...
pub fn view_fibonacci_loop(ctx: &ScViewContext, f: &FibonacciLoopContext) {
    let n = f.params.n().value();
    let a: u64 = 1;
    let b: u64 = 1;
    let mut i: u64 = 2;
...
pub fn view_test_sandbox_call(ctx: &ScViewContext, f: &TestSandboxCallContext) {
    ...
    f.results
        .sandbox_call()
        .set_value(&get_chain_info.results.description().value());
}

pub fn func_loop(ctx: &ScFuncContext, f: &LoopContext) {
    let n = f.params.n().value();
    for i in 0..n {
